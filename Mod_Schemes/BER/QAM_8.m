...
while (error<500)               %500 errors before calculting BER
    ...
    R=T+N+M*1j;                  %Received bits with effect of noise
        ...
        if ((real(R)>2) && (imag(R)>0))     %Hard decision to identify the received bits
            R=3+1j;
            r=[0 0 0];
        end
        if ((real(R)<2) && (real(R)>0) && (imag(R)>0))       %Hard decision to identify the received bits
            R=1+1j;
            r=[0 0 1];
        end
        if ((real(R)>-2) && (real(R)<0) && (imag(R)>0))       %Hard decision to identify the received bits
        ...
        if ((real(R)<0) && (real(R)>-2) && (imag(R)<0))       %Hard decision to identify the received bits
            R=-1-1j;
            r=[1 1 1];
        end
         if ((real(R)<2) && (real(R)>0) && (imag(R)<0))       %Hard decision to identify the received bits
